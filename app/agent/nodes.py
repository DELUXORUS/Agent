...
from app.agent.state import AgentState, IntentClassification, MovieFilter
...
async def search_for_recommended_node(state: AgentState, llm: ChatOpenAI) -> dict[str, Any]:
...
async def search_for_guess_node(state: AgentState, llm: ChatOpenAI) -> dict[str, Any]:
    last_user_message = state["messages"][-1].content
...
async def general_chat(state: AgentState, llm: ChatOpenAI) -> dict[str, Any]:
    messages = [SystemMessage(content=prompts["SYSTEM_GENERAL_CHAT_PROMPT"])] + state["messages"]
